...
class VKLoginHelper:
	...
	def _real_login(self, login, password):
		...
		payload_r = self._post_assert_response(login_url, {}, ref="https://vk.com", expected_status_code=200, redirect_helper=True)
	...
	# webpage getters
	def _post_assert_response(self, url, data, ref = None, expected_status_code=200, redirect_helper=False, *args, **kvargs):
		headers = self.post_headers
		self._reference_headers(headers, url, ref)
		r = self.session.post(url, data, headers=headers, allow_redirects=not redirect_helper, *args, **kvargs)
		print(r.status_code)
		if redirect_helper:
			while r.status_code == 302 or r.status_code == 307:
				url = r.headers["location"]
				self._reference_headers(headers, url, ref)
				r = self.session.post(url, data, headers=headers, allow_redirects=False, *args, **kvargs)
				print(r.status_code)
